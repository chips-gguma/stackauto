...
if __name__ == '__main__': 
    try:
        ...
        buy_percent = 0.19   # 전체 가용 자금에서 몇 %를 매수할 것인지 (각 종목당)
        ...
        while True:
            ...
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0) # LP(유동성 공급자) 활동 시간
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) # 자동 매도 시간 
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0)  # 종료
